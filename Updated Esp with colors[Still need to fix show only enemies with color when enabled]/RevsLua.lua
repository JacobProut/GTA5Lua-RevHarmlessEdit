...
--Health Added Code--
  ---Adds CheckBox/Assigns npcEspHealthCB---
        ...
        for i, ped in ipairs(allPeds) do
            if ENTITY.DOES_ENTITY_EXIST(ped) and not PED.IS_PED_A_PLAYER(ped) and PED.IS_PED_HUMAN(ped) and not PED.IS_PED_DEAD_OR_DYING(ped, true) then
                ...
                if distance <= npcEspDistance then
                    ...
                    if success and npcEspBoxCB and (not npcEspShowEnemiesCB or pedEnemy or npcEspColorByDistanceCB) then
                    ...
                    if success and npcEspTracerCB and npcEspColorByDistanceCB and (not npcEspShowEnemiesCB or pedEnemy) then
                     ...
                     if success and npcEspColorByDistanceCB and npcEspBoxCB and (not npcEspShowEnemiesCB) then
                        local camCoords = CAM.GET_GAMEPLAY_CAM_COORD()
                        local distanceToCam = calculate_distance(pedCoords.x, pedCoords.y, pedCoords.z, camCoords.x, camCoords.y, camCoords.z)
                        local boxSize = 2 * (1 / distanceToCam) -- Calculate boxSize here
                        local color = calculate_color(distance)
                        local minThickness = 0.001
                        local thickness = math.max(minThickness, 0.0015 * (1 / distanceToCam))
                    ...
                    -- Color by distance --
                    --if success and npcEspColorByDistanceCB and npcEspShowEnemiesCB and pedEnemy then
                    --    local camCoords = CAM.GET_GAMEPLAY_CAM_COORD()
                    --    local distanceToCam = calculate_distance(pedCoords.x, pedCoords.y, pedCoords.z, camCoords.x, camCoords.y, camCoords.z)
                    --    local boxSize = 2 * (1 / distanceToCam) -- Calculate boxSize here
                     --   local color = calculate_color(distance)
